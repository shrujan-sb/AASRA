...
export async function POST(req: Request) {
  try {
    ...
    const roadLabel = body.road || blockedIds.join(", ") || "corridor";
    ...
    return NextResponse.json({
      ...
      model: clerk?.model,
      headline: clerk?.headline ?? `Corridor update for ${roadLabel}. ${picks.length} mission(s) reassigned.`,
    ...
    return NextResponse.json({
      ok: true,
      studied: false,
      headline: "Desk recorded the corridor. Missions stay on the board until a free unit is clear.",
      alternatives: [],
      picks: [],
      notes: [],
      incidents: [],
